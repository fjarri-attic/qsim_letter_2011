...
The Wigner method allows us to reconstruct the spin noise distribution in the plane orthogonal to the mean spin (\figref{fig:riedel-rotation}a) and the result can be compared with the two-mode model (\figref{fig:riedel-rotation}b).
...
The calculated value of the quantum noise at maximum squeezing ($-10.7\pm1.8\un{dB}$) is slightly greater than the two-mode model prediction ($-12.8\un{dB}$).
