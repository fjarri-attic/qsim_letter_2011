...
\begin{figure}
	...
	Plot (b) shows the dependance of the normalized variance on the rotation angle $\theta$ for
	the Wigner method (solid red line) and the two-mode method (dashed blue line, data taken from~\cite{Riedel2010})
	}}
	\label{fig:riedel-rotation}
\end{figure}

\revision{
Wigner representation shows qualitative agreement with other approximate methods like the two-mode model with losses~\cite{Li2009}.
...
A BEC of 1250 atoms was prepared in a cigar-shaped trap with $\omega_x = \omega_y = 2 \pi \times 109\un{Hz}$,
$\omega_z = 2 \pi \times 500\un{Hz}$.
After the first $\pi / 2$-pulse, the potentials for both components were split, leading to the period of separation.
...
The result of the Wigner method is plotted against the one given by the two-mode model in \figref{fig:riedel-rotation}.
