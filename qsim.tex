...
\maketitle
	...
	\bogdansremark{$\delta^{(D)}$ --- do we really need to specify dimension number? it can be deduced from the arguments. Same for $d^D$ in integrals.}
...
\begin{equation}
	\hat{O}_{\jvec}^{(n)} =
	O_{\jvec}^{(n)} \left( \widehat{\Psivec} \right) =
...
\begin{equation}
	\frac{\partial}{\partial t} \left\langle
		\widehat{\Psi}_{j} \left( \xvec \right)
	\right\rangle =
	-\left\langle \hat{\Gamma}_{j} \left( \xvec \right) \right\rangle
\end{equation}
	\bogdansremark{How was this derived?}
where we have introduced a nonlinear loss operator $\hat{\Gamma}_{j} \left( \xvec \right) =
\Gamma_{j} \left( \widehat{\boldsymbol{\Psi}} \left( \xvec \right) \right)$
as:
\begin{equation}
	\hat{\Gamma}_{j} \left( \xvec \right) \equiv
	\sum_{n,\ivec} \kappa_{\ivec}^{(n)}
	\frac{\partial\hat{O}_{\ivec}^{(n)\dagger}	\left( \xvec \right)}
		{\partial \hat{\Psi}_{j}^{\dagger} \left( \xvec \right)}
	\hat{O}_{\ivec}^{(n)} \left( \xvec \right) \,.
\end{equation}
...
with:
\begin{equation}
	D_{ij} = \sum_{n,\mathbf{k}} \kappa_{\ivec}^{(n)}
...
\begin{equation}
	\frac{d\psi_{j}}{dt} = iK_{j} \psi_{j} - ig_{ij} |\psi_{i}|^{2} \psi_{j} - \Gamma_{j} +
		\sum_{n,\ivec} \gamma_{\ivec,j}^{(n)} \zeta_{\ivec}^{(n)} (\xvec,t)
\end{equation}

Here the linear unitary evolution of the wave-function, is described by:
\begin{equation}
	K_{j} = \hbar \nabla^{2}/2m - \omega_{j} - V_{j} \left( \xvec \right)
\end{equation}
while $\zeta_{\ivec}^{(n)}(t,\xvec)$ is a complex,
...
\begin{equation}
	\gamma_{\ivec,j}^{(n)} \left( \boldsymbol{\psi} \right) =
	\sqrt{\kappa_{\ivec}^{(n)}}
	\frac{\partial O_{\mathbf{k}}^{(n)}}{\partial\psi_{j}}
\end{equation}
