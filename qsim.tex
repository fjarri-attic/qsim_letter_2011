...
\label{eqn:SDE}
	...
	\Gamma_{j} + \sum_{n,\ivec} \beta_{\ivec,j}^{(n)} \zeta_{\ivec}^{(n)} (\xvec,t).
\end{equation}
...
\begin{equation}
	\beta_{\ivec,j}^{(n)} \left( \Psivec \right) =
	\sqrt{\kappa_{\ivec}^{(n)}}
...
%	\vskip 0.2cm
	...
	Classical GPE (red lines) and Wigner calculations (blue lines)
...
As an illustration consider~\figref{fig:squeezing}, which shows calculations of squeezing parameter
for optically trapped $\ket{1,+1}$ and $\ket{2,-1}$ BEC in Ramsey sequence.
Different inter-component scattering lengths were used in order to find optimal regime
with significant squeezing, lasting for maximum amount of time.
