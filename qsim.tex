...
\begin{split}
	\widehat{H} / \hbar & = \int d^{D}\xvec \left\{
		...
		\Omega_{ij}(t) \widehat{\Psi}_{i}^{\dagger} \widehat{\Psi}_{j}
	\right. \\
	& + \left.
		(\omega_i + V_i(\xvec) / \hbar) \widehat{\Psi}_{i}^{\dagger} \widehat{\Psi}_{i} +
...
$V_{i}$ is the external trapping potential for spin $i$,
...
\begin{figure}
	...
	%	\vskip 0.2cm
	\imagetop{\includegraphics{figures_generated/rephasing_squeezing.eps}} & \imagetop{(b)}
	\end{tabular}
