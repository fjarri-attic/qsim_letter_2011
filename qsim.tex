...
In free-space calculations it is important maintain this mode truncation,
...
\label{eqn:SDE-1}
	\frac{d\Psi_j}{dt} = -i \left(
		K_{ij} + U_{ij} \lvert \Psi_i \rvert^2
	\right) \Psi_j.
\end{equation}
...
$\Psi_s(\xvec, t_0) = \Psi_s^c(\xvec) + \Delta \Psi_s(\xvec)$, where:
$\left\langle \Delta \Psi_s(\xvec) \Delta \Psi_u^*(\xvec^{\prime}) \right\rangle = \delta_{su} \delta^D \left( \xvec - \xvec^{\prime} \right) / 2.$
...
\end{equation}
where $n$ is the number of interacting particles, and $\lvec$ is a vector indicating the spins that are coupled, and have introduced local Liouville loss terms,
\begin{equation}
	\mathcal{L}_{\lvec}^{(n)} \left[ \hat{\rho} \right] =
...
$\hat{O}_{\lvec}^{(n)} = \hat{O}_{\lvec}^{(n)} \left( \widehat{\Psivec} \right) = \widehat{\Psi}_{l_{1}} \left( \xvec \right) \widehat{\Psi}_{l_{2}} \left( \xvec \right) \ldots \widehat{\Psi}_{l_{n}} \left( \xvec \right),$
...
\label{eqn:SDE}
	\frac{d\Psi_{j}}{dt} =
		- i\left( K_{ij} + U_{ij} \lvert \Psi_{i} \rvert^{2} \right) \Psi_{j}
		- \Gamma_{j}
...
\begin{equation}
	\Gamma_{i} = \sum_{n,\lvec}
		\kappa_{\lvec}^{(n)}
		\frac{\partial O_{\lvec}^{(n)*} \left(\Psivec\right)}{\partial\Psi_{i}^{*}\left(\xvec\right)}
		O_{\lvec}^{(n)}\left(\xvec\right),
\end{equation}
and $\zeta_{\lvec}^{(n)}(\xvec, t)$ is a corresponding complex, stochastic delta-correlated Gaussian noise with
\begin{equation}
	\left\langle
...
*****************************NEED AN EXPLICIT LINK TO SIMULATIONS HERE*****************************************
...
which mean that some detuning of the magnetic field is essential to reduce
...
atom interferometry experiments can be readily included.Wigner method of
simulating BEC dynamics is quantitatively accurate for a range of interferometric experiments involving large numbers of atoms and multiple loss mechanisms.
